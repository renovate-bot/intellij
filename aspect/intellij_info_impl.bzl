...
def collect_java_toolchain_info(target, ide_info, ide_info_file, output_groups):
    ...
    if not java_common.JavaToolchainInfo in target:
        return False
    toolchain_info = target[java_common.JavaToolchainInfo]

    # TODO(https://github.com/bazelbuild/bazel/issues/6015): improve toolchain type-check
    if type(toolchain_info) != "JavaToolchainSkylarkApiProvider":
        return False
